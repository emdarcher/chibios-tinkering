...
#define USE_PWM_STUFF FALSE
#define USE_ADC_STUFF FALSE
...
/* function to calculate temp in Celsius using function 
in datasheet and user manual for the STM32 internal
temperature sensor 

equation:
C = {(V25 - VSense)/Avg_Slope} + 25
STM32F103C8T6 datasheet values for Vdda of 3.3V
            Min     Typ     Max
            -------------------
TL          -       +-1     +-2     degC
Avg_slope   4.0     4.3     4.6     mV/degC
V25         1.34    1.43    1.52    V
tstart      4       -       10      us
TS_temp     -       -       17.1    us

C = ((1.43V - Vsense)/4.3mV)+25
C = ((1.43V - Vsense)/0.0043V)+25
C = ((1430000 - uVsense)/4300)+25
C = (((1430000 - uVsense)+107500)/4300)
cent-degC = ((((1430000 - uVsense)+107500)*100)/4300)



*/
#define SPEC_VDDA_uV 3300000UL
#define uV25 (((1430000UL * MY_VDDA_UV)/ SPEC_VDDA_uV))
const uint32_t c_uV25 = uV25;
#define AVG_SLOPE_uV (((4300UL * MY_VDDA_UV )/ SPEC_VDDA_uV))
const uint32_t c_AVG_SLOPE_uV = AVG_SLOPE_uV;
#define ADD_25_VAL (25*AVG_SLOPE_uV)
const uint32_t c_ADD_25_VAL = ADD_25_VAL;
static int32_t uV_to_centiDegC_sensor(uint32_t in_uV){
    int32_t uVsense = (int32_t)in_uV;
    int32_t temp_calc = (int32_t)((((c_uV25 - uVsense)+c_ADD_25_VAL)*100L)/((int32_t)c_AVG_SLOPE_uV)); 
    return temp_calc;
}
...
*/
static int32_t uV_to_centiDegC_STLM20(uint32_t in_uV){
...
static __attribute__((noreturn)) msg_t ADCreadout(void *arg){
    ...
    while(TRUE){
        ...
        int32_t scDegC = uV_to_centiDegC_sensor(adc12_to_uV(adc_avg_sensor,MY_VDDA_UV));
        int32_t cDegC = uV_to_centiDegC_STLM20(adc12_to_uV(adc_avg_STLM20,MY_VDDA_UV));
        ...
        sdWrite(&SD1,(uint8_t *)"internal sensor: ",17);
        char * scDegCStr = centiDeg_to_str(scDegC);
        sdWrite(&SD1,(uint8_t *) scDegCStr, 7);
        sdWrite(&SD1,(uint8_t *)" STLM20 Vout: ",14);
